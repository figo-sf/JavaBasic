 ...
 * the License.
 */
package com.figo.generator.engine;

import com.baomidou.mybatisplus.core.toolkit.StringUtils;
import com.figo.generator.config.ConstVal;
import com.figo.generator.config.GlobalConfig;
import com.figo.generator.config.TemplateConfig;
import com.figo.generator.config.builder.ConfigBuilder;
import com.figo.generator.config.po.TableInfo;
import com.figo.generator.util.FileUtils;
import com.figo.generator.util.RuntimeUtils;
...
public abstract class AbstractTemplateEngine {
    ...
    protected void outputEditVue(@NotNull TableInfo tableInfo, @NotNull Map<String, Object> objectMap) {
        // MpController.java
        String controllerPath = getPathInfo(ConstVal.CONTROLLER_PATH);
        if (StringUtils.isNotBlank(tableInfo.getControllerName()) && StringUtils.isNotBlank(controllerPath)) {
            getTemplateFilePath(TemplateConfig::getController).ifPresent(controller -> {
                String entityName = tableInfo.getEntityName();
                String controllerFile = String.format((controllerPath + File.separator +"edit"+ entityName+".vue"), entityName);
                outputFile(new File(controllerFile), objectMap, "/templates/vue.ftl");
            });
        }
    }
    ...
    protected void outputIndexVue(@NotNull TableInfo tableInfo, @NotNull Map<String, Object> objectMap) {
        ...
        if (StringUtils.isNotBlank(tableInfo.getControllerName()) && StringUtils.isNotBlank(controllerPath)) {
            getTemplateFilePath(TemplateConfig::getController).ifPresent(controller -> {
                String entityName = tableInfo.getEntityName();
                String controllerFile = String.format((controllerPath + File.separator + entityName+"index.vue"), entityName);
                outputFile(new File(controllerFile), objectMap, "/templates/indexVue.ftl");
            });
        }
    }

    protected void outputTs(@NotNull TableInfo tableInfo, @NotNull Map<String, Object> objectMap) {
        // MpController.java
        String controllerPath = getPathInfo(ConstVal.CONTROLLER_PATH);
        if (StringUtils.isNotBlank(tableInfo.getControllerName()) && StringUtils.isNotBlank(controllerPath)) {
            getTemplateFilePath(TemplateConfig::getController).ifPresent(controller -> {
                String entityName = tableInfo.getEntityName();
                String controllerFile = String.format((controllerPath + File.separator + StringUtils.firstToLowerCase(entityName)+".ts"),entityName);
                outputFile(new File(controllerFile), objectMap, "/templates/ts.ftl");
            });
        }
    }

    protected void outputRouter(@NotNull TableInfo tableInfo, @NotNull Map<String, Object> objectMap) {
        // MpController.java
        String controllerPath = getPathInfo(ConstVal.CONTROLLER_PATH);
        if (StringUtils.isNotBlank(tableInfo.getControllerName()) && StringUtils.isNotBlank(controllerPath)) {
            getTemplateFilePath(TemplateConfig::getController).ifPresent(controller -> {
                String entityName = tableInfo.getEntityName();
                String controllerFile = String.format((controllerPath + File.separator + StringUtils.firstToLowerCase(entityName)+".txt"),entityName);
                outputFile(new File(controllerFile), objectMap, "/templates/router.ftl");
            });
        }
    }
    ...
    public AbstractTemplateEngine batchOutput() {
        try {
            ...
            tableInfoList.forEach(tableInfo -> {
                ...
                outputEditVue(tableInfo, objectMap);
                outputIndexVue(tableInfo, objectMap);
                outputTs(tableInfo, objectMap);
                outputRouter(tableInfo, objectMap);
